...
#[macroquad::main("Static Trimesh 3D")]
async fn main() {
    let world = World::new();
    ...
    world.add_unique(BallState::default()).unwrap();
    ...
    loop {
        ...
        world.run_with_data(ball_spawner, get_frame_time()).unwrap();
...
fn ball_spawner(
    ...
    mut ball_state: UniqueViewMut<BallState>,
) {
    if ball_state.balls_spawned >= ball_state.max_balls {
        return;
    }

    ball_state.seconds_until_next_spawn -= delta_seconds;
    ...
    entities.add_entity(
        (&mut bodies_builder, &mut colliders_builder),
        (rigid_body, collider),
    );
